...
function salvarControle() {
    ...
    ### Define o arquivo de configuração do controle.
    if [ -f "$HOME/.emulationstation/es_input.cfg" ]; then
        JOY_FILE_CONF="$HOME/.emulationstation/es_input.cfg"
    else
        JOY_FILE_CONF="$SHARE_INIT/system/.emulationstation/es_input.cfg"
    fi
    ...
    ES_INPUTS=`xml sel -T -t -m "/inputList/inputConfig[@deviceGUID='$JOY_ID']/*" -v "concat(@name,'|', @id)" -n "$JOY_FILE_CONF"`
...
if ! [ -f "$HOME/configs/raine/rainex_sdl.cfg" ]; then
    ...
    if [ -f "$HOME/recalbox.conf" ]; then
        XLANG="$(cat "$HOME/recalbox.conf" | grep ^system.language= | cut -d '=' -f 2)"
        case $XLANG in
...
# salva esta informação no arquivo $HOME/recalbox.conf então este trexo do codigo
...
if [ "$EMULADOR" == 'auto' ]; then
    if [ -f "$HOME/recalbox.conf" ] && \
       [ "$(cat $HOME/recalbox.conf | grep neogeocd.emulator=)" ] && \
       [ "$(cat $HOME/recalbox.conf | grep neogeocd.emulator= | cut -d'=' -f 2)" != 'auto' ]; then
        EMULADOR="$(cat $HOME/recalbox.conf | grep neogeocd.emulator= | cut -d'=' -f 2)"
        if [ "$(cat $HOME/recalbox.conf | grep neogeocd.core=)" ] && \
           [ "$(cat $HOME/recalbox.conf | grep neogeocd.core= | cut -d'=' -f 2)" != 'auto' ]; then
            CORE="$(cat $HOME/recalbox.conf | grep neogeocd.core= | cut -d'=' -f 2)"
        fi
    fi
fi
...
if [ -f "$HOME/recalbox.conf" ]; then
    if [ "$(cat $HOME/recalbox.conf | grep neogeocd.smooth= | cut -d'=' -f 2)" == '0' ]; then
        sed -i s/'^ogl_filter = .*/ogl_filter = 9728/' "$HOME/configs/raine/rainex_sdl.cfg"
    elif [ "$(cat $HOME/recalbox.conf | grep neogeocd.smooth= | cut -d'=' -f 2)" == '1' ]; then
        sed -i s/'^ogl_filter = .*/ogl_filter = 9729/' "$HOME/configs/raine/rainex_sdl.cfg"
    elif [ "$(cat $HOME/recalbox.conf | grep global.smooth= | cut -d'=' -f 2)" == '0' ]; then
        sed -i s/'^ogl_filter = .*/ogl_filter = 9728/' "$HOME/configs/raine/rainex_sdl.cfg"
    elif [ "$(cat $HOME/recalbox.conf | grep global.smooth= | cut -d'=' -f 2)" == '1' ]; then
        sed -i s/'^ogl_filter = .*/ogl_filter = 9729/' "$HOME/configs/raine/rainex_sdl.cfg"
    fi
fi
...
if [ -f "$HOME/recalbox.conf" ]; then
    FILTRO_GLOBAL="$(cat $HOME/recalbox.conf | grep ^global.shaderset= | cut -d'=' -f 2)"
    FILTRO_NEOCD="$(cat $HOME/recalbox.conf | grep ^neogeocd.shaderset= | cut -d'=' -f 2)"
fi
...
if [ -f "$HOME/recalbox.conf" ]; then
    RES_NEOCD="$(cat $HOME/recalbox.conf | grep neogeocd.ratio= | cut -d'=' -f 2)"
    RES_GLOBAL="$(cat $HOME/recalbox.conf | grep global.ratio=   | cut -d'=' -f 2)"
