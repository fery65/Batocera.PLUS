...
if [ "$SYSTEM" == 'neogeocd' ]; then
    ...
    if [ -z "$NGCD_EMULATOR" ]; then
        NGCD_EMULATOR='auto'
    fi

    case $NGCD_EMULATOR in raine-cdz|raine-front-load|raine-top-load|raine-unibios|auto)
        EMULATOR=$NGCD_EMULATOR
        unset NGCD_EMULATOR
        ...
        case $NGCD_CORE in raine2018|raine2015|auto)
            CORE=$NGCD_CORE
            unset NGCD_CORE ;;
        esac
