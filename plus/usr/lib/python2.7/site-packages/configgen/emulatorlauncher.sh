...
if [ -f "$SHARE/system/.emulationstation/es_settings.cfg" ]; then
    THEME_NAME="$(cat "$SHARE/system/.emulationstation/es_settings.cfg" | grep '<string name="ThemeSet" value=' | cut -d '"' -f 4)"
    if ! [ "$THEME_NAME" ]; then
...
if [ -d "$SHARE/system/.emulationstation/themes/$THEME_NAME" ]; then
    THEME_DIR="$SHARE/system/.emulationstation/themes/$THEME_NAME"
elif [ -d "$SHARE_INIT/system/.emulationstation/themes/$THEME_NAME" ]; then
    THEME_DIR="$SHARE_INIT/system/.emulationstation/themes/$THEME_NAME"
else
    THEME_DIR="$SHARE_INIT/system/.emulationstation/themes/batocera-plus"
fi

if [ -f "$THEME_DIR/loadscreen/$SYSTEM.png" ]; then
    /usr/bin/retroarch \
    -L '/usr/lib/libretro/imageviewer_libretro.so' \
    --config "$SHARE_INIT/system/configs/retroarch/retroarchcustom.cfg" \
    "$THEME_DIR/loadscreen/$SYSTEM.png" &
elif [ -f "$THEME_DIR/loadscreen/$SYSTEM.jpg" ]; then
    /usr/bin/retroarch \
    -L '/usr/lib/libretro/imageviewer_libretro.so' \
    --config "$SHARE_INIT/system/configs/retroarch/retroarchcustom.cfg" \
    "$THEME_DIR/loadscreen/$SYSTEM.jpg" &
else
    /usr/bin/retroarch \
    -L '/usr/lib/libretro/imageviewer_libretro.so' \
    --config "$SHARE_INIT/system/configs/retroarch/retroarchcustom.cfg" \
    "$SHARE_INIT/system/.emulationstation/themes/batocera-plus/loadscreen/loadscreen.png" &
fi
...
function runInFolder {
    ...
    # exemplo  msdos e pcgames
    case "$SYSTEM" in
        pc|dos|pcgames) return 0 ;;
    esac
...
case $SYSTEM in
    ...
    snescd)         SYSTEM='snes'       ;;
    satellaview)    SYSTEM='snes'       ;;
    sufamiturbo)    SYSTEM='snes'       ;;
    n64dd)          SYSTEM='n64'        ;;
...
if [ "$SYSTEM" == 'pokemini' ]; then
    if [ "$EMULATOR" == 'auto' ]; then
        EMULATOR='libretro'
    fi
    if [ "$CORE" == 'auto' ]; then
        CORE='pokemini'
